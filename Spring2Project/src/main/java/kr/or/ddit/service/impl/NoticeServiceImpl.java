...
public class NoticeServiceImpl implements INoticeService {
	...
	public ServiceResult noticeDelete(HttpServletRequest req, int boNo) {
		...
		// 게시글 번호에 해당하는 공지사항 게시글 정보 가져오기(파일정보들을 가져오기 위해서)
		...
		if (status > 0) { // 삭제성공
			...
			try {
				if (noticeFileList != null) {
					// D:\KYS\00.TOOL\02.SPRING\workspace_spring2\.metadata....\resources\notice\130
					// /UUID_원본파일명
					// 두개의 구간으로 split 된다.
	...
	public ServiceResult idCheck(String memId) {
		...
		if (member != null) { // memId를 넘겻을때 member에 있다면 EXIST를
			result = ServiceResult.EXIST;
		} else { // 없으면 NOTEXIST를 반환
	...
	public ServiceResult signup(HttpServletRequest req, NoticeMemberVO memberVO) {
		...
		// 회원가입 시, 포로필 이미지로 파일을 업로드 하는데 이때 업로드할 서버 경로
		...
		String proFileImg = ""; // 회원정보에 추가될 프로필 이미지 경로를 담아줄 공간
		...
		if (status > 0) {
			// 한명의 회원이 등록 될 때에 하나의 권한을 무조건 가질수 있도록 권한 등록(스프링 시큐리티 적용시)
		...
		try {
			if (folder.exists()) { // 경로가 존재한다면
				...
				for (int i = 0; i < folderFileList.length; i++) {
					...
					}else { // 폴더안에 있는 파일이 폴더일때
						// 폴더라면 재귀함수를 호출해서 폴더안으로 들아갈수있도록 해준다.
	...
	@Override
	public String findId(@RequestParam String memEmail, String memName) {
		
		String memId = loginMapper.findId(memEmail,memName);
	...
	public String findPw(String memId, String memEmail, String memName) {
		String memPw = loginMapper.findPw(memId,memEmail,memName);
		return memPw;
	}
