...
public class AOPController {
	 ...
	 *		: 횡단 관심사에 해당하는 부분(메소드 실행 시작 시간 출력 등)을 분리해서 한 곳으로 모오는 것을 의미
	 *		- @Component 
	 *		: @Aspect와 짝꿍
	 ...
	 *		- 관점 지행 프로그래밍(Aspect Oriented Programming)을 의미하는 약자입니다.
	 *
	 *		1-1) 관점 지향 프로그래밍
	 *
	 ...
	 *
	 *			- 횡단 관심사의 분리(Separation Of Cross-Cutting Concerns)
	 ...
	 *
	 *			- 의존 관계 등록 (pom.xml 설정)
	 *				> aspectjrt(이미 등록 되어 있음)
	 *				> aspectjweaver
	 *
	 *			- 스프링 AOP 설정
	 *				> root-context.xml 설정
	 *				: AOP를 활성화 하기 위한 태그를 작성합니다.
	 *
	 *			## 스프링 AOP
	 *				스프링 AOP는 동작 시점이 여러가지가 있지만(컴파일 시점, 클래스 로딩 시점, 런타임 시정..)
	 *				그중, 런 타임 시점에 프로시 객체를 생성 후 기능을 삽입
	 *				> 스프링 AOP의 프록시는 메소드의 오버라딩 (인터페이스 기반의 참좀) 개념으로 동작 하므로,
	 *				메소드 실행 시점 시 동작
	 *
	 *	2. 포인트 컷 표현식
	 *
	 ...
	 *			- execution 지시자를 활용해 포인트컷을 표현한 것 입니다.
	 *
	 *			- 포인트 컷 표현 요소
	 ...
	 *					+		|	클래스명 뒤에 붙여 쓰며, 해당 클래스와 해당 클래스의 서브 클래스, 혹은 구현 클래스
	 *							|	모두를 의미한다.
	 *			───────────────────────────────────────────────
	 *
	 ...
	 *		AOPController와 조인포인트로 설정한 Service 부분들이 basePackage로 설정된 하위 자원들의
	 *		중복으로 빈등록이 되지 않아 AOP가 활성화 되지 않는 문제가 발생할 수 있습니다.
	 *		AOP를 적용시키는데에 있어서 패키지 구조 설계에 따른 중복으로 인해 빈등록이 되지 않는 오류를 최소화 하기
	 *		위해서는 퍼스펙티브를 스프링으로 변경 후, Project Explorer를 활용해 Spring Bean으로 등록되어
	 *		있는 내역들 중, @Aspect가 적용되어 있는 AOPController가 빈으로 등록되어 있는지를 체크하고
	 *		체크했다면 혹 중복 되어 빈으로 등록되고 있지는 않은지 체크하고, 각각의 빈들이 중복되어 만들어지지 
	 *		않고 정상적으로 만들어지는 거라면 위빙 포인트가 생성되어 타겟이 실행되기 전 AopProxy가 활성화 되어
	 *		인터페이스 기반의 프록시로서 역할을 할 수 있도록 true인지 체크한다.
	 *
	 *	### AOP 프록시
	 *
	 ...
	 *		그리고 응답을 나가는  부분을 잡아야하는 처리가 필요 시, 응답을 잡아서 처리 후 최종 응답을 내보낸다.
	 *
	 *				[클라이언트]	>>>	[프록시]	>>>	[타켓]
	 *
	 */
	...
	public void startLog(JoinPoint jp) {
		...
		log.info("[@Before] tergetObject : " + targetObject);
	...
	public void logReturnig(JoinPoint jp) {
		log.info("[AfterReturning] logReturnig");
		log.info("[AfterReturning] logReturnig : " + jp.getSignature());
	 ...
	 * 		ProceddingJoinPoint 객체로 받아서 around 어드바이스가 컨트롤 시 활용할 수 있도록 해줌
	 */
	
	@Around("execution(* kr.or.ddit.service.IBoardService.*(..))")
	public Object itemLog(ProceedingJoinPoint pjp) throws Throwable {
		// 메소드 실행 직전 시간 체킹
		...
		log.info("[@Around] : " + pjp.getSignature());
