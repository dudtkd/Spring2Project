...
public class FileUploadController01 {
	 ...
	 * 				> pom.xml commons 종류의 io,fileupload추가
	 * 
	 * 			2) 웹 컨테이너 설정
	 * 				> web.xml 서블릿 표준 버전 3.1로 설정
	 * 				> multipart-config 활성화 (web.xml의 servlet태그 내 설정)
	 * 
	 * 			3) 스프링 웹 설정
	 * 				> servlet-context.xml
	 ...
	 * 						> StandardServletMultipartResolverBean 등록
	 * 					> web.xml에 설정
	 * 						> multipart-config(servlet 태그 안에 설정)
	 * 				
	 * 				- CommonsMultipartResolver 사용시 설정, (우리가 사용 할 방식)
	 ...
	 * 
	 * 2.이미지 업로드
	 * - 한 개의 이미지를 업로드 하는 기본 파일 업로드 기능을 구현 합니다.
	 * 	
	 * 		# 파일 업로드 구현 설명
	 * 
	 ...
	 * 			- 파일 업로드 등록 기능 서비스 인터페이스 만들기
	 * 			- 파일 업로드 등록 기능 서비스 클래스 만들기
	 ...
	 * 			- 파일 업로드 수정 화면 서비스 인터페이스 만들기
	 * 			- 파일 업로드 수정 화면 서비스 클래스 만들기
	...
	public String itmeRegister(Item item , Model model) throws Exception {
	...
	public ResponseEntity<byte[]> displayFile(int itemId){
		...
		// 확장자를 던져서 확장자와 일치하는 MimeType(미디어 타입)을 리턴한다.
