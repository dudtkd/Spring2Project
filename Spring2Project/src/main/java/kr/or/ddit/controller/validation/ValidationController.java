...
public class ValidationController {
	 ...
	 * 1. 입력값 검증
	 * 
	 * - 스프링 MVC Bean Validation 기능을 이용해 요청 파라미터 값이 바인딩된 도메인 클래스 (또는 커멘드 클래스) 의 입력값을
	 * 검증을 한다.
	 * 
	 * [환경설정] # 의존 관계 정의 - 입력값 검증을 위한 의존 라이브러리를 추가한다. - pom.xml에서
	 * hibernate-validator 추가
	 * 
	 * # 입력값 검증 활성화 - Member 클래스로 넘어가서 임시 테스트로 serId, userName에 규칙을 화성화 한다 - 이때, 규칙을
	 * 활성화 하기 위해서 사용할 어노테이션이 있다. > @validated를 지정한다. > 입력값 검증 대상의 도메인 클래스 직후에
	 * BindingResult를 정의 한다. >> BindingResult에는 요청 데이터의 바인딩 오류의 입력값 검증 오류 정보가 저장된다.
	 * 
	 * # 입력값 검증 환경설정 순서 1. 입력값 검증을 위한 의존 라이브러리를 추가 2. 입력값 검증 활성화 > 활성화를 할 도메인
	 * 클래스안에 @validatd 어노테이션을 지정한다. 3. 도메인 클래스 내 필드에다가 검증을 위한 어노테이션들로 데이터 검증을
	 * 설정한다.(@NotBlank, @Size 등등) 4. 에러를 받을 BindingResult를 설정한다. ( 컨트롤러 메소드 내에
	 * 설정합니다.)
	 */
	...
	/*
	 * 2. 입력값 검증 결과 - 입력값 검증 대상의 도메인 클래스 직후에 BindingResult를 정의한다.
	 * 
	 * # Bindingresult에는 요청 데이터의 바인딩 에러와 입력값 검증 에러 정보가 저장된다. 1) 에러 정보 확인을 위한
	 * Bindingresult 메서드
	 * 
	 * hasError() - 에러가 발생한 경우 true 를 반환한다. hasGlobalErrors() - 객체 레벨의 에러가 발생한 경우
	 * true 를 반환한다. hasFieldErrors() - 필드 레벨의 에러가 발생한 경우 true를 반환한다.
	 ...
	 * 		@Null					: null 인지 검사한다.
	 * 		@NotBlank				: 문자열이 null 아니고 trim한 길이가 0보다 크다는 것을 감시한다.
	 * 		@NotEmpty				: 문자열이 null 이거나 비어있는지 검사한다.
