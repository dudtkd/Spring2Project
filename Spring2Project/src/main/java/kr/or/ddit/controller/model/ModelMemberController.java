...
import kr.or.ddit.controller.MemberController;
...
public class ModelMemberController {
	 ...
	 * - Model 객체 이용
	 * 
	 * 1) 매개변수에 선언된 Model객체의 addAttribute() 메서드를 호출하여 데이터를 뷰(View)에 전달한다.
	 */
	...
	// 2) Model 객체에 자바빈즈 클래스 객체를 값으로만 전달할 때는 뷰에서 참조할 이름을 클래스명의 앞글자를
	// 소문자로 변환하여 처리한다.
