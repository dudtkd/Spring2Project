...
public class AjaxMemberController {
	...
	public ResponseEntity<String> ajaxRegister05(String userId,@RequestBody Map<String,String> map){
		// userId는 쿼리스트링에 담겨져 오는 데이터이기 때문에, 일반적인 방식으로도 데이터를 받을 수 있지만, // 소수 파라미터를 받기 위해서는 map 객체를 이용한다.				
		// password는 요청 본문에서 데이터를 바인딩해 받아오지 못하므로 null이 출력된다.
		// 비동기 통신이면서 요청본문에 설정되어넘어오는 단일 파라미터 같은 경우에는 @RequestBody 어노테이션을 설정하고
	...
	public ResponseEntity<String> register07(@RequestBody List<Member> memberList){
		// 동기방식과 비동기 방식을 처리하는데에 있어서 List타입을 운용하는걸 꼭 기억합시다.!
		// 비동기 처리시, List 컬렉션으로 데이터를 받을때에는 @RequestBody 라는 어노테이션을 이용하여 바인딩 할 수 있다.
