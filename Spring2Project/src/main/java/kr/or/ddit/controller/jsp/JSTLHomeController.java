...
public class JSTLHomeController {
	 ...
	 *		<c:choose>		| 여러조건을 처리할때 사용한다. 
	 * 		<c:when>		| 여러 조건을 지정하고 조건과 일치하는 처리 내용을 구현한다. <c:choose>요소에서 사용 
	 ...
	 *			default		| Object	| value의 결과값인 null인 경우 출력할 값(대신 출력됨)
	 *			────────────────────────────────────────────────────────────
	 *
	 *			2) <c:set>
	 *				- JSP에서 사용할 변수를 설정한다.
	 ...
	 *
	 *			9) <c:forEach>
	 *				- 컬렉션이나 배열의 각 항목을 처리 할 때 사용한다.
	 *
	 *				속성		|	타입		|	설명
	 *			────────────────────────────────────────────────────────────
	 *			var			| String	| 몸체에서 사용할 EL 변수 이름
	 *			items		| Object	| 반복 처리할 데이터
	 *			varStatues	| String	| 루프 상태를 저장할 EL 변수 이름
	 *			begin		| int		| 시작 인덱스 값
	 *			end			| int		| 끝 인덱스 값
	 *			step		| int		| 인덱스 증분 값				
	 *			────────────────────────────────────────────────────────────
	 ...
	 *				- 구분자로 구분된 각각의 토큰을 처리할 때 사용한다.
 	 ...
 	 *			delims		| String	| 구분자
 	 *			varStatus	| String	| 루프 상태를 지정할 EL 변수 이름
 	 *			begin		| int		| 시작 인덱스 값
 	 *			end			| int		| 끝 인덱스 값
 	 *			step		| int		| 인덱스 증분 값			 
 	 *			────────────────────────────────────────────────────────────
	...
	// c:catch
	// EL에서 발생한 에러가 아니라, 자바식에서 바생한 에러 정보기 때문에 var속성에 담긴 에러 정보를 확인 할 수 있습니다.
	...
	// c:when, c:otherwise
	// when과 otherwise는 c:choose안에서 활용 할 수 있다.
