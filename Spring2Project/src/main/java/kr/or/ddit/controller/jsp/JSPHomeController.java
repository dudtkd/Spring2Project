...
public class JSPHomeController {
	 ...
	 * 		3. 커스텀 태그 라이브러리
	 * 
	 * 			- 스크립트 요소가 많아지면 많아질수록 JSP 코드는 복잡해 진다.
	 * 			- 위 문제를 해결하는 한 가지 방법은 커스텀 태그를 사용하는 것 이다.
	 ...
	 * 				== (eq)	| 같은 값인지 비교한다
	 * 				!= (nq)	| 다른 값인지 비교한다
	 * 				<= (lq)	| 왼쪽이 작거나 같은 값인지 비교한다.
	 * 				>= (gq)	| 왼쪽이 크거나 같은 값인지 비교한다.
	 * 				<  (lt)	| 왼쪽이 작은 값인지 비교한다.
	 * 				>  (gt)	| 왼쪽인 큰 값인지 비교한다
	 * 			──────────────────────────────────────────
	 * 
	 ...
	 *			|| (or) 	| 두 피연산 중 하나 또는 모두 true이면 bool true를 반환하고, 그렇지 않으면 false를 반환한다. 
	 *			!  (not)	| 해당 피연사자의 의미를 반대로 바꾼다. 
	 * 			───────────────────────────────────────────────────────────────────────
	 * 
	 */
	...
	public String home0104(Model model) {
		...
		// Map 안에 들어있는 데이터를 출력하기 위해서는 "속성명.키명" 또는 "속성명[키명]" 으로 꺼낼 수 있다.
