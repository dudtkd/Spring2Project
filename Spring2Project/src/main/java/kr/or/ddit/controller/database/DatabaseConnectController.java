...
public class DatabaseConnectController {
	 ...
	 * 
	 * 			- Oracle win32_lgR2_client와 OracleXE112-Win64로 오라클 클라이언트, 서버를 설치
	 * 
	 * 			# 오라클 클라이언트 및 서버 설치 참고
	 * 			- https://junesker.tistroy.com/79 (오라클 클라이언트 설치)
	 * 			- https://junesker.tistroy.com/80 (오라클 서버 설치)
	 * 			
	 ...
	 * 			- https://junesker.tistroy.com/81 (오라클 SqlDeveloper 설치)
	 * 		
	 * 		3. 데이터소스 설정
	 * 
	 ...
	 * 				- query
	 * 					: ResutSetExtractor, RowCallbackHandler와 함께 조회할 때 사용한다.
	 * 				- update
	 ...
	 * 
	 * 				- 데이터소스 설정
	 * 					> jdbcTemplate 클래스를 빈으로 정의( 우리는 따로 하지 않음)
	 * 
	 * 				- 설정 예시
	 ...
	 * 				# 게시판 구현 설명
	 * 
	 * 					기본적인 CRUD를 바탕으로 한 게시판 구현입니다.
	 * 					등록, 수정, 삭제, 상세보기 등의 페이지를 구현하여 JDBC방식 기반의 CRUD를 작성합니다.
	 * 					JDBC의 방식은 기존 PrepareStatement를 활용한 방식과 동일하므로 넘어가도록 합니다.!
	 * 
	 * 			6. JAP(Java Persistence API)
	 * 
	 * 				- JPA는 자바 표준 ORM입니다.
	 ...
	 * 				등을 처리해준다.
	 * 				기존 SQL 쿼리를 이용하여 서비스를 처리할때에 필드가 추가되면 필드와 관련되어 있는 기능, sql, 추가된 필드
	 * 				등을 수정해야하는 번거로움이 발생하지만 JAP는 필드만 추가후 테이블 정보와 매핑만 시켜주면 JPA가 자동으로
	 * 				분석을 진행한다.
	 * 				그리고 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다.(JPA는 Sql을 직접 건들이지 않고 함수와
	 * 				같은 기능들로만 쿼리를 요청하고 데이터베이스와 통신하기 때문)
	 * 
	 * 				JAP는 JPA구현체인 hibernate를 이용하여 사용합니다. 그러기 위해선 관련 의존관계를 등록 후 사용해야 합니다.
	 ...
	 * 
	 * 				*** 기업 면접에서 ORM은 뭐 쓰셧어요? 라는 질문을 받는 경우가 있습니다.
	 * 					- 저는 ibatis를 사용 했습니다, 저는 mybatis를 사용 했습니다 와 같은 대답을 하면 됩니다.
	 * 
	 * 		7. 마이바티스
	 * 
