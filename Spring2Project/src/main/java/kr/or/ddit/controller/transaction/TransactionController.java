...
public class TransactionController {
	 ...
	 * 				회원은 회원권한 테이블이 각각 개별로 존재하지만 회원 정보를 저장할 때 반드시 회원권한 정보를
	 * 				동시에 저장해야 한다.
	 * 				클래스나 메소드의 대해 @Transactional 어노테이션을 부여하여 트랜잭션을 적용할 수 있다.
	 * 
	 * 		# 서비스 구현 객체에 트랜잭션 적용
	 * 			
	 * 			- 회원정보를 저장하다가 실패하거나 회원 권한 정보를 저장하다가 실패 하면 회원 테이블과 회원 권한
	 ...
	 * 			롤백이 되지 않고 데이터베이스에 데이터가 등록됩니다. 왜?? 난 분명 롤백 처리를 했는데도 불구하고
	 * 			데이터가 등록되는것을 볼 수 있습니다.
	 * 
	 * 			그 이유는 스프링 프레임워크에서 @Transactional은 기본적으로 Checked Exception에
	 * 			대해서는 롤백처리를 하지 않도록 설계되어 있습니다. 기본적으로 스프링에서 트랜잭션 처리는 
	 * 			RuntimeException계열(UnCkecked Exception)이라면 RollBack처리를 합니다.
	 * 
	 * 			여기서, 트랜잭션으로 국한된 롤백 정책이 아니라 스프링 프레임우커ㅡ에서의 기본 정책에 대한 내용일
	 * 			뿐입니다. 실제로 '트랜잭션 롤백 처리','CheckedException VS UnCheckedException'
	 * 			등 검색 해보면 잘못된 정보들로 복사/붙여넣기 되어 있는 내용이 수두룩하여 많은 에러 정보를 공유하고
	 ...
	 * 			(많은 블로그 글을 100% 믿지 말길..., 혹여나 블로그를 관리하고 있는 사람이라면 검증된 부분으로만 글을
	 * 			작성해주길 부탁합니다.)
	 * 
	 * 	# Exception(예외)와 Error(에러)
	 * 
	 * 		- Exception : 개발 알고리즘에서 발생하는 오류로 개발자가 작성한 코드의 발생하므로 예외를 상황에
	 * 						맞춰 처리할 수 있습니다.
	 * 		- Error : 시스템에서 발생하는 심각한 수준의 에러로 개발자가 미리 예측하여 대응할 수 없기 때문에
	 * 					예외 처리에 대한 부분을 신경쓰지 않아도 됩니다.
	 * 
	 ...
	 *				Commit 하더라도 자신의 트랜잭션에서 처리한 내용만 사용하는것을 의미
	 *
	 ...
	 *					> 하나의 트랜잭션 처리가 이루어진 변경 내용이 Commit 된 후, 다른 트랜잭셔에서 조회가
	 *						가능합니다.
	 ...
	 *			- SERIALIABLE : Dirty read, Non-repeatable Read 및 Phantom Read가 방지됨
	 *							을 나타내는 상수입니다.
	 *				> Phantom Read가 발생하지 않습니다.
	 *				> 거의 사용하지 않는 옵션
	 *
	 *		2) propagation(전파옵션)
	 *			- 기존 진행중인 트랜잭션 외에 추가적으로 진행중인 트랜잭션이 존재할 때 추가적인 트랜잭션에
	 *				대해서 어떻게 처리할지에 대한 설정
	 *			- 추가적인 트랜잭션을 기존 트랜잭션에 포함 시켜 함께 처리할 수도 있고, 추가적인 트랜잭션처럼
	 *				별도의 트랜잭션으로 추가 할 수도 있고 다른 트랜잭션처럼 진행되다 에러를 발생 시킬 수도 있다.
	 *
	 *			2-1) 옵션
	 ...
	 *				> 읽기 전용에 대한 힌트를 분석 할 수 없는 트랜잭션인 경우 throw를 던지지 않고 조용히
	 *					힌트를 무시하고 쓰기와 같은 트랜잭션이 실행된다.
	 *				> 그래서 꼭! 무조건! 읽기전용이라고 할 수 없다.
	 *
	 *		4) timeout (트랜잭션 제한시간)
	 *			- 기본값은 -1로 무제한이다.
