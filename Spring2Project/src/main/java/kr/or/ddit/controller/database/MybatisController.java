...
public class MybatisController {
	 ...
	 * 				- 마이바티스의 위치 설정은 root-context.xml의 'sqlSessionFactiory' 설정 시,
	 * 					property 요소로 적용합니다.
	 * 
	 * 			4) 관련 테이블 생성
	 * 				- board 테이블 생성
	 ...
	 * 		2. Mapper 인터페이스
	 * 
	 * 			- 인터페이스 구현을 mybatis-spring에서 자동으로 생성할 수 있다.
	 * 
	 * 			1) 마이바티스 구현
	 * 
	 * 				1-1) Mapper 인터페이스
	 ...
	 * 				- 여기까지 확인 
	 * 
	 *				- 게시판 삭제 기능 컨트롤러 메소드 만들기 (crudModify:post)
	 *				- 게시판 삭제 기능 서비스 인터페이스 만들기
	 ...
	 *				- mybatisAlias가 설정되어 있지 않는 경우엔 타입으로 설정하고자 하는 타입의 형태를 패키지명이 포함되어 있는 구조로 설정해야 한다.
	 ...
	 *
	 *			'_'가 포함되어 있는 데이터베이스 컬럼명을 Camel기법 셋팅으로 인해서 bo_no이 아니라 boNo으로 처리된다.
	 *
	 *			1) 마이바티스 설정
	 *
	 *			1-1) mybatisAlias.xml 설정
	 ...
	 *			- as boardNo, asregDate로 컬렁명을 대체하는 alias가 설정되어 있는 경우엔 as 부분을 삭제
	 *
	 *		5. 기본키 취득
	 *
	 *			- 마이바티스는 useGeneratedKeys 속성을 이용하여 insert할 때 데이터 베이스 측에서 채변된 기본키를
	 *			취득할 수 있다.
	 *
	 *			1) 마이바티스 설정
	 *
	 ...
	 *			사용하고 board라는 자바빈즈 객체에 담겨 최종 컨트롤러 까지 넘어온다.
	 *
	 *			*** seq.board.nextval 대신 현재의 seq번호를 가져오기 위해 currval를 사용하게 되는 경우
	 *				이때, 주의사항이 있습니다.
	 *
	 *				- select seq_board.currval from dual
	 *				> 위 select 쿼리를 사용 시, currval를 사용하는데 있어서 사용 불가에 대한 에러가 발생 할 수 있습니다.
	 ...
	 *				> select last_number from user_sequences where sequence_naem = '시퀀스 명';
	 *				> select last_number from user_sequences where sequence_naem = 'seq_board';
	 *				쿼리로 가져오면 되지만 권고 사항 절대 아님!
	 *
	 *			2-2) 컨트롤러 메소드에서 crudRegister 부분의 등록이 되고나서 리턴되는 방향성을 수정
	 *				- 월래는 완료페이지로 전환되지만, board에 담겨있는 boardNo를 이용하여 상세보기로 전환
	 *
	 *		6. 동적 SQL
	 *
	 ...
	 *		7. 일대 다 관계 테이블 매핑
	 *
	 *			- 마이바티스 기능을 활용하여 매핑 파일을 적적하게 정의하면 일대다 관계 테이블 매핑을 쉽게 처리할 수 있다.
	 *
	 *			1) 게시판 구현 설명
	 *
	 ...
	 *			- 여기까지 확인 
	 *
	 *			- 회원 목록 화면 컨트롤러 메소드 만들기 (crudMemberRegister:get)
