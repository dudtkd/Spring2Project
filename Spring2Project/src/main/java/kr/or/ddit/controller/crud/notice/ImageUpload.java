...
public class ImageUpload {
	...
	public String imageUpload(
		...
		// CKEidtor4 특정 버전 이후부터 html 형식의 데이터를 리턴하는 방법에서
		...
		if(file != null && file.getSize() > 0 && StringUtils.isNotBlank(file.getName())) {
			...
			if(file.getContentType().toLowerCase().startsWith("image/")) {
				if(file.getSize() > limitSize) {	// 업로드 한 파일 사이즈가 최대 크기보다 클 때
					 ...
					 * {
					 * 		"uploaded" : 1,
					 * 		"fileNae"  : "XXXXXXXX-XXXXX_YYYYY.jpg",
					...
					try {
						...
						// 업로드 경로로 설정된 폴더 구조가 존재하지 않는 경우, 파일을 복사 할 수 없으므로
						...
						json.addProperty("fileNaem", fileName);
