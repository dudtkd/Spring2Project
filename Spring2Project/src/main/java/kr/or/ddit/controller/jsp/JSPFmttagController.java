...
public class JSPFmttagController {
	 ...
	 *					기본값은 number이다. currency는 속해 있는 국가코드에 맞는 통화 형식이 부여된다.
	 *
	 * 
	 * 		2) <fmt:parseNumber>
	 * 			- 문자열을 숫자로 변환한다.
	...
	// 그래서 type 속성과 일치하여 값을 파싱 할 수 있다.
	...
	// 5) type 속성을 date로 지정하여 날짜 포맷팅한다.
	...
	// dateStyle 속성을 각 스타일로 지정하여 문자열을 Date 객체로 변환한다.
	@RequestMapping(value = "/home0401", method = RequestMethod.GET)
	public String home0401(Model model) {
		String dateValueShort = "20. 2. 1";			// style short 형태 지정
		String dateValueMedium = "2020. 2. 1";		// style medium 형태 지정
		String dateValueLong = "2019년 2월 1일 (금)";	// style long 형태 지정
		String dateValueFull = "2019년 2월 1일 금요일";	// style full 형태 지정
		
		// 각 dateStyle로 지정된 값이 페이지로 넘어가 parsing 이 진행될때, parseDate내에
